 ...
 * 
 <!-- options-end -->
 *  
 * @author Andres L. Suarez-Cetrulo (andres.suarez-cetrulo@ucd.ie)
...
public class WilsonEditing
  ...
  protected int m_NearestNeighbors, default_nn = 3;
...
public Enumeration<Option> listOptions() {
  ...
  public void setOptions(String[] options) throws Exception {  
    ...
    setNumberOfNearestNeighbors(m_kNN.length() != 0 ? Integer.parseInt(m_kNN): this.default_nn);

    String wilsonMode = Utils.getOption('J', options);
    if (wilsonMode.length() != 0) setWilsonMode(Integer.parseInt(wilsonMode));
    else  setWilsonMode(0);
  }  
  ...
  public String[] getOptions() {
    ...
    result.add("-M");
    result.add("" + this.m_NearestNeighbors);
  ...
  public void setWilsonSubPopulation(Instances inst) {
    if (m_subPopulationMode){
      this.m_subPopulationMode = false;
      this.m_subpopulation = inst;
     } else System.out.println("The Wilson mode has to be equal to 1");  
  }
  ...
  public Instances getWilsonSubPopulation(Instances inst) {
    if (this.m_subPopulationMode) return this.m_subpopulation;	 
    else {
		System.out.println("The Wilson mode has to be equal to 1");  
		return null;
    }
  }
  ...
  public void setWilsonMode(int value) {
	switch (value) {
		case 0: this.m_subPopulationMode = false;
			break;
		case 1: this.m_subPopulationMode = true;
			break;
		default:
  ...
  public int getWilsonMode() {
	  return this.m_subPopulationMode ? 1 : 0;
  }
  ...
  public Instances getSubPopulation() {
	  return this.m_subPopulationMode ? this.m_subpopulation : null;
  }
   ...
   */
  public void setNumberOfNearestNeighbors(int value) {
    // if (value >= this.default_nn) this.m_NearestNeighbors = value;
    // else System.err.println("At least 3 neighbors necessary!");
   ...
   * 	instance structure (any instances contained in the object are ignored - only the structure is required).
  ...
  public boolean batchFinished() throws Exception {
    ...
    System.out.println("## STARTING ENN ##");
    // Do Wilson, and clear the input instances.
    if (!this.m_FirstBatchDone) computeENN();
    flushInput();
  ...
  /**
   * This method performs the selection of the instances to be added into the reduced population
   * 
   * @param neighbours the neighbors of the targeted instance 
  ...
  public boolean compareNeighbours(Instances neighbours, Instance i){	
	double halfOfKnn = (double) this.m_NearestNeighbors / 2.0;  
	int numMatches = 0;
	
	for (int j = 0; j < neighbours.numInstances(); j++) {
		if(neighbours.get(j).classValue()==i.classValue()){
		    if(this.m_debug) System.out.println("NEIGHBOUR " + neighbours.get(j) + " MATCHES");
		    numMatches++;
		} if(numMatches > halfOfKnn){
		    if(this.m_debug) System.out.println("ADDED ON REDUCED DATASET => "+neighbours.get(j));
		    return true;
	    }				
	} return false;
  ...
  protected void printInfo(){
	    System.out.println("Using " + this.m_NearestNeighbors + " neighbours");
	    if(getWilsonMode()==0) System.out.println("Using Wilson Editing");
	    else System.out.println("Using Wilson Editing only under sub-population");	  
  }
   ...
   * 			on input instances
   */
  protected void computeENN() throws Exception {
	...
	if(this.m_debug) System.out.println("\n Begin of WilsonEditing Filter \n");
	if (this.m_subPopulationMode) { //wilsonMode == 1 only subpopulation							
		reduced.clear();
		EnumData = this.m_subpopulation.enumerateInstances();
		...
		while (EnumData.hasMoreElements()) {
		    ...
		    neighbours = m_NNSearch.kNearestNeighbours(i, this.m_NearestNeighbors);
		    
		    // 4 compare neighbours			    
		    if(compareNeighbours(neighbours, i)) reduced.add(i);			    	

		} if(this.m_debug) {
			System.out.println(this.m_subpopulation.numInstances()+" SVs de la it. anterior");
	...
	if(this.m_subPopulationMode) {
		if(this.m_debug) System.out.println("Adding dataset in subpopulation mode");
		
		// The sub-population mode also returns the input data set
		EnumData = getInputFormat().enumerateInstances();
		while(EnumData.hasMoreElements()) push((Instance) EnumData.nextElement());
